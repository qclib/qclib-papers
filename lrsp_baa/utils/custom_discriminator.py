...
# Altered from the original repository qiskit_machine_learning
...
from qiskit_machine_learning.algorithms import DiscriminativeNetwork
...
class CustomPyTorchDiscriminator(DiscriminativeNetwork):
    """
    Discriminator based on PyTorch
    """

    def __init__(self, n_features: int = 1, n_out: int = 1, device: str ='cpu') -> None:
        """
        Args:
            n_features: Dimension of input data vector.
            n_out: Dimension of the discriminator's output vector.
            device: The device to be used in the discriminator net
        ...
        from ._pytorch_discriminator import CustomDiscriminatorNet

        self._discriminator = CustomDiscriminatorNet(self._n_features, self._n_out)
        self._discriminator.to(torch.device(device))
        self.device = device
    ...
    def save_model(self, snapshot_dir: str, file_name: str = "discriminator.pt"):
        """
        ...
        """
        torch.save(self._discriminator, os.path.join(snapshot_dir, file_name))
    ...
    def get_label(self, x, detach=False):  # pylint: disable=arguments-differ
        ...
        if detach:
            return self._discriminator.forward(x).to('cpu').detach().numpy()
        else:
    ...
    def train(
        ...
        real_batch = torch.tensor(real_batch, dtype=torch.float32, device=self.device)
        real_batch = Variable(real_batch)
        real_prob = np.reshape(real_prob, (len(real_prob), 1))
        # Adding cuda
        real_prob = torch.tensor(real_prob, dtype=torch.float32, device=self.device)
        ...
        # adding cuda -- 
        error_real = self.loss(prediction_real, torch.ones(len(prediction_real), 1, device=self.device), real_prob)
        error_real.backward()

        # Train on Generated Data
        generated_batch = np.reshape(generated_batch, (len(generated_batch), self._n_features))
        generated_batch = torch.tensor(generated_batch, dtype=torch.float32, device=self.device) # my alteration
        generated_prob = np.reshape(generated_prob, (len(generated_prob), 1))
        generated_prob = torch.tensor(generated_prob, dtype=torch.float32, device=self.device) # my alteration
        prediction_fake = self.get_label(generated_batch)

        # Calculate error and back propagate
        error_fake = self.loss(
            prediction_fake, torch.zeros(len(prediction_fake), 1, device=self.device), generated_prob
        )
        error_fake.backward()
        ...
        self._ret["loss"] = loss_ret.to('cpu').detach().numpy()
        params = []

        for param in self._discriminator.parameters():
            params.append(param.data.to('cpu').detach().numpy())
